...
# Legacy condensed set — production live inventory is 21 profiles in open_smell2
# ─────────────────────────────────────────────
SCENT_PROFILES = {
...
# Canonical header. Ground-truth columns (is_injected / injected_key /
# injected_condition) sit BEFORE the prediction columns so every row reads
# truth | prediction | features — a labeled benchmark, not just coverage.
LOG_HEADER = [
    "timestamp", "sex", "year_of_birth", "age",
    "is_injected", "injected_key", "injected_condition",
    "top_match", "category", "confidence", "severity",
    "alert_triggered", "action", "raw_vocs"
]

def init_log():
    """Create the log with the canonical header.

    If a log already exists with a DIFFERENT header (e.g. the pre-ground-truth
    schema, or the mixed 8-/11-column file), it is rolled to a timestamped
    backup so new labeled rows are never appended onto an incompatible schema.
    """
    if os.path.exists(LOG_FILE):
        with open(LOG_FILE, "r", newline="") as f:
            first = f.readline().rstrip("\n")
        if first.split(",") != LOG_HEADER:
            backup = f"opensmell_log.pre_gt_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
            os.rename(LOG_FILE, backup)
            print(f"  [logger] existing log had a legacy schema — archived to {backup}")
...
def log_result(reading, matches, alert):
    ...
    # ── GROUND TRUTH ──────────────────────────────────────────────
    # The simulator already tracks which profile it injected on this cycle
    # (reading["__injected_key__"]). Persist it so accuracy — not just
    # coverage/match rate — can be computed. injected_key is None on
    # background (non-injected) cycles, whose true label is "Healthy /
    # Background" and whose correct prediction is "No match".
    injected_key = reading.get("__injected_key__")
    is_injected = injected_key is not None
    injected_condition = (
        SCENT_PROFILES[injected_key]["condition"]
        if is_injected else "Healthy / Background"
    )
    ...
    with open(LOG_FILE, "a", newline="") as f:
        ...
        writer.writerow([
            ...
            patient.get("age", "—"),
            is_injected,
            injected_key if is_injected else "—",
            injected_condition,
